  ...
  try {
    switch (cmd) {
      ...
      case "menuv": {
        ...
        // Build one combined menu text from all sections
        const pages     = buildMenuSections(prefix);
        const fullMenu  = buildMenu(prefix) + "\n\n" +
          pages.map(page => page.join("\n\n")).join("\n\n");

        if (menuVideo) {
          // Send video with the header as caption (WhatsApp caps captions at ~1024 chars)
          await sock.sendMessage(from, {
            video:       menuVideo,
            caption:     buildMenu(prefix),
            mimetype:    "video/mp4",
            gifPlayback: false,
          }, { quoted: msg });
          // Send the full command list as one combined message right after
          await sock.sendMessage(from, { text: pages.map(page => page.join("\n\n")).join("\n\n─────────────────────\n\n") }, { quoted: msg });
        } else {
          await reply(sock, msg, fullMenu);
        }
        break;
      }
