  ...
  try {
    switch (cmd) {
      ...
      case "setwelcome": {
        if (!isGroup) { await reply(sock, msg, "❌ Group only."); break; }
        if (!isAdminUser) { await reply(sock, msg, "🔒 Admin only."); break; }
        const swArg = (text || "").trim().toLowerCase();
        if (swArg === "on" || swArg === "off") {
          const swEnabled = swArg === "on";
          premium.setWelcomeCard(from, swEnabled);
          await reply(sock, msg, `🎨 Welcome cards *${swEnabled ? "✅ enabled" : "❌ disabled"}* for this group.${swEnabled ? "\n\nA branded image card will be generated and sent when new members join." : ""}`);
          break;
        }
        if (!text) { await reply(sock, msg, `Usage:\n• *${prefix}setwelcome on/off* — toggle welcome card images\n• *${prefix}setwelcome [msg]* — set custom welcome text\n\nUse {{name}} and {{group}} as placeholders.`); break; }
      ...
      case "autotranscribe": {
        if (!isGroup) { await reply(sock, msg, "⚠️ This command is only for groups."); break; }
        if (!isAdminUser) { await reply(sock, msg, "⚠️ Group admins only."); break; }
        const atArg = (text || "").toLowerCase().trim();
        if (!["on", "off"].includes(atArg)) {
          await reply(sock, msg, `🎙 *Auto Transcribe*\n\nUsage: *${prefix}autotranscribe on/off*\n\nAutomatically transcribes voice notes sent in this group.`);
          break;
        }
        const atEnabled = atArg === "on";
        premium.setAutoTranscribe(from, atEnabled);
        await reply(sock, msg, `🎙 Auto-transcription *${atEnabled ? "✅ enabled" : "❌ disabled"}* for this group.`);
        break;
      }
      ...
      case "remind": {
        if (!text) {
          await reply(sock, msg,
            ...
            `Examples:\n• ${prefix}remind 2h call John\n• ${prefix}remind 30m take medicine\n• ${prefix}remind 1h30m meeting\n\n` +
            `Time formats: \`2h\` \`30m\` \`1h30m\` \`45s\`\n\n` +
            `View reminders: *${prefix}reminders*\nCancel: *${prefix}cancelremind [id]*`
          );
          break;
        }
        const [remTimeExpr, ...remMsgParts] = text.split(/\s+/);
        const remMessage = remMsgParts.join(" ");
        if (!remMessage) { await reply(sock, msg, `❌ Please provide a message. Example: *${prefix}remind 2h call John*`); break; }
        const remResult = premium.addReminder(senderJid, remTimeExpr, remMessage);
        if (remResult.error) { await reply(sock, msg, remResult.error); break; }
        const remMin = Math.round(remResult.ms / 60000);
        const remH   = Math.floor(remMin / 60), remM = remMin % 60;
        const remETA = remH > 0 ? `${remH}h ${remM}m` : `${remMin}m`;
        await reply(sock, msg,
          `✅ *Reminder set!*\n\n` +
          `📌 ID: \`${remResult.id}\`\n` +
          `📝 Message: ${remMessage}\n` +
      ...
      case "reminders": {
        const myRems = premium.getReminders(senderJid);
        if (!myRems.length) { await reply(sock, msg, "📭 You have no pending reminders."); break; }
        const remList = myRems.map(r => {
          const inMs   = r.fireAt - Date.now();
          const inMin  = Math.round(inMs / 60000);
          const inH    = Math.floor(inMin / 60), inM = inMin % 60;
          const eta    = inH > 0 ? `${inH}h ${inM}m` : `${inMin}m`;
          return `• \`${r.id}\` — ${r.message} _(in ${eta})_`;
        }).join("\n");
        await reply(sock, msg, `⏰ *Your Reminders:*\n\n${remList}\n\n_Cancel: ${prefix}cancelremind [id]_`);
        break;
      }

      case "cancelremind": {
        if (!text.trim()) { await reply(sock, msg, `❌ Usage: *${prefix}cancelremind [id]*`); break; }
        const cancelled = premium.cancelReminder(senderJid, text.trim());
        await reply(sock, msg, cancelled ? "✅ Reminder cancelled." : "❌ Reminder not found (check ID with `.reminders`).");
        break;
      }

      case "catchup": {
        const cuN = parseInt(args[0], 10) || 50;
        await reply(sock, msg, `🔄 Summarising last ${cuN} messages...`);
        const cuSummary = await premium.catchUpSummary(from, cuN);
        await reply(sock, msg, `📰 *Catch-Up Summary*\n${"─".repeat(28)}\n\n${cuSummary}`);
        break;
      }

      case "ocr": {
        const ocrInner = msg.message?.ephemeralMessage?.message || msg.message || {};
        const ocrImgMsg = ocrInner.imageMessage;
        const ocrQuoted = msg.message?.extendedTextMessage?.contextInfo?.quotedMessage?.imageMessage;
        if (!ocrImgMsg && !ocrQuoted) {
          await reply(sock, msg, `🔍 *Image OCR*\n\nUsage:\n• Send an image with caption \`.ocr\`\n• Or reply to an image with \`.ocr\``);
          break;
        }
        await reply(sock, msg, "🔍 Extracting text from image...");
        const ocrMsgTarget = ocrImgMsg
          ? msg
          : { key: { remoteJid: from, id: msg.message.extendedTextMessage.contextInfo.stanzaId, fromMe: false, participant: msg.message.extendedTextMessage.contextInfo.participant }, message: { imageMessage: ocrQuoted } };
        const { downloadMediaMessage } = require("@whiskeysockets/baileys");
        const ocrBuf = Buffer.from(await downloadMediaMessage(ocrMsgTarget, "buffer", {}));
      ...
      case "mood": {
        if (!isGroup) { await reply(sock, msg, "⚠️ This command is only for groups."); break; }
        await reply(sock, msg, "😊 Analysing group mood...");
        const moodResult = await premium.groupMoodReport(from, 50);
        if (moodResult.error) { await reply(sock, msg, moodResult.error); break; }
        await reply(sock, msg,
          `${moodResult.emoji} *Group Mood Report*\n` +
          `${"─".repeat(28)}\n\n` +
          `🏷 *Sentiment:* ${moodResult.sentiment}\n` +
          `📝 *Reason:* ${moodResult.reason}\n\n` +
          `_Analysed last 50 messages — NEXUS-MD ⚡_`
        );
        break;
      }
      ...
      case "setwelcomecard": {
        if (!isGroup) { await reply(sock, msg, "⚠️ This command is only for groups."); break; }
        if (!isAdminUser) { await reply(sock, msg, "⚠️ Group admins only."); break; }
        const wcArg = (text || "").toLowerCase().trim();
        if (!["on", "off"].includes(wcArg)) {
          const cur = premium.isWelcomeCardEnabled(from);
          await reply(sock, msg,
            `🎨 *Welcome Cards*\n\nUsage: *${prefix}setwelcome on/off* _(or ${prefix}setwelcomecard on/off)_\n\nCurrently: ${cur ? "✅ ON" : "❌ OFF"}\n\nWhen ON, a branded image card is generated and sent when new members join.`
          );
          break;
        }
        const wcEnabled = wcArg === "on";
        premium.setWelcomeCard(from, wcEnabled);
        await reply(sock, msg, `🎨 Welcome cards *${wcEnabled ? "✅ enabled" : "❌ disabled"}* for this group.`);
        break;
      }
      ...
      case "setdigest": {
        if (!isGroup) { await reply(sock, msg, "⚠️ This command is only for groups."); break; }
        if (!isAdminUser) { await reply(sock, msg, "⚠️ Group admins only."); break; }
        const sdOnOff = (args[0] || "").toLowerCase().trim();
        if (!["on", "off"].includes(sdOnOff)) {
          const dEnabled = premium.isDigestEnabled(from);
          const dTime    = premium.getDigestTime(from);
          await reply(sock, msg,
            `📰 *Daily Digest*\n\nUsage:\n• *${prefix}setdigest on* — enable at ${dTime}\n• *${prefix}setdigest off* — disable\n• *${prefix}setdigest on 07:30* — enable at specific time (24h)\n\nCurrently: ${dEnabled ? `✅ ON (${dTime})` : "❌ OFF"}\n\nThe digest includes weather, a news headline, football fixtures, and a motivational quote.`
          );
          break;
        }
        const sdEnabled = sdOnOff === "on";
        const sdTimeArg = args[1] && /^\d{1,2}:\d{2}$/.test(args[1]) ? args[1].padStart(5, "0") : null;
        premium.setDigest(from, sdEnabled, sdTimeArg);
        const sdTime = premium.getDigestTime(from);
        await reply(sock, msg, `📰 Daily Digest *${sdEnabled ? `✅ enabled (fires at ${sdTime})` : "❌ disabled"}* for this group.`);
        break;
      }

      case "digest": {
        if (!isGroup) { await reply(sock, msg, "⚠️ This command is only for groups."); break; }
        await reply(sock, msg, "⏳ Fetching today's digest...");
        const digestMsg = await premium.buildDailyDigest(from);
        await reply(sock, msg, digestMsg);
        break;
      }
